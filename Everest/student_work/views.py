...
from .forms import StudentLoginForm, AdminLoginForm,Math_update_Form,Lesson_update_Form,Math_sub_Form,Lesson_sub_Form ,StudentSignUpForm,AssignmentForm
...
def all_assignments(request):
    ...
    print("now", now) 
    print("formatted_current_time : " ,formatted_current_time )
    ...
    # print("formatted_current_time_local",formatted_current_time_local)


    math_assignments = Assignments.objects.filter(math_send_time__lt=now).order_by('-math_send_time')
    lesson_assignments = Assignments.objects.filter(lesson_send_time__lt=now).order_by('-lesson_send_time')
...
def submit_math_assignment(request, assignment_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            submission, created = Submission.objects.get_or_create(student=request.user, assigmnets=assignment)
            submission.math_sub_link = submission_link
            submission.math_sub_date = formatted_current_time
            
            deadline_datetime = datetime.datetime.strptime(submission.math_sub_date, '%Y-%m-%dT%H:%M:%S%z')
...
def submit_lesson_assignment(request, assignment_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            deadline_datetime = datetime.datetime.strptime(submission.lesson_sub_date, '%Y-%m-%dT%H:%M:%S%z')
...
def admin_tracker(request):
    Student_tracking_data = ( Submission.objects.values('student__username')
                             .annotate(
...
def student_tracker(request):
    ...
    math_submissions = Submission.objects.filter(
            ...
            math_sub_link__isnull=False,
            math_sub_date__isnull=False
