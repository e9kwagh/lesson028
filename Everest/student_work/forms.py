...
class Math_sub_Form(forms.Form):
    math_submission_link = forms.URLField(
        ...
        required=True,
        widget=forms.TextInput(attrs={'class': 'form-control mt-2', 'placeholder': 'Link'})
    )
...
class Lesson_sub_Form(forms.Form):
    lesson_submission_link = forms.URLField(
        label='Enter Submission Link',
        required=True,
        widget=forms.TextInput(attrs={'class': 'form-control mt-1', 'placeholder': 'Enter your submission link'})
    )
...
class AssignmentForm(forms.ModelForm):
    class Meta:
        model = Assignments
        fields = ['title', "description",'math_send_time', 'math_deadline', 'lesson_send_time', 'lesson_deadline', 'math_question_link', 'lesson_question_link']
        widgets = {
            'math_send_time': forms.DateTimeInput(attrs={'type': 'datetime-local'}, format='%Y-%m-%dT%H:%M'),
            'math_deadline': forms.DateTimeInput(attrs={'type': 'datetime-local'}, format='%Y-%m-%dT%H:%M'),
            'lesson_send_time': forms.DateTimeInput(attrs={'type': 'datetime-local'}, format='%Y-%m-%dT%H:%M'),
            'lesson_deadline': forms.DateTimeInput(attrs={'type': 'datetime-local'}, format='%Y-%m-%dT%H:%M'),
        }
    ...
    def clean_profile_pic(self):
        ...
        if profile_pic:
            ...
            if profile_pic.size > max_size:
                raise forms.ValidationError('File size exceeds the maximum allowed size.')
...
class AssignmentSubmissionForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not math_sub_link and not lesson_sub_link:
            raise forms.ValidationError('At least one of Math Submission Link or Lesson Submission Link is required.')
